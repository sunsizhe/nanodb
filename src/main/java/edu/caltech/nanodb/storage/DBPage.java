...
public class DBPage implements Pinnable, AutoCloseable {
    ...
    public BigDecimal readNumeric(int position) {
        // NUMERIC/DECIMAL values are stored as a 1-byte scale, and then a
        // 15-byte twos-complement integer.  These are combined to produce a
        // BigDecimal value.

        int scale = readByte(position);
    ...
    public void writeNumeric(int position, BigDecimal value) {
        // NUMERIC/DECIMAL values are stored as a 1-byte scale, and then a
        // 15-byte twos-complement integer.  These are combined to produce a
        // BigDecimal value.
        ...
        if (scale > 127 || scale < -128) {
            throw new IllegalArgumentException("Cannot handle BigDecimals " +
                "with scale outside [-128, 127]; got " + scale);
        }
        BigInteger intValue = value.unscaledValue();

        byte[] data = intValue.toByteArray();
        if (data.length > 15) {
            throw new IllegalArgumentException("Cannot handle BigDecimals " +
            ...
            System.arraycopy(data, 0, data15, 15 - data.length, data.length);
            data = data15;
        }
        assert data.length == 15;

        writeByte(position, scale);
        write(position + 1, data);
    }


    public LocalDate readDate(int position) {
        // Dates are stored as a 32-bit signed int value indicating
        // distance from the "epoch date" of January 1, 1970.
        return LocalDate.ofEpochDay(readInt(position));
    }


    public void writeDate(int position, LocalDate value) {
        // Dates are stored as a 32-bit signed int value indicating
        // distance from the "epoch date" of January 1, 1970.

        // TODO:  May want to identify any truncation issues that can occur
        //        when casting the date to an int.
        writeInt(position, (int) value.toEpochDay());
    }


    public LocalTime readTime(int position) {
        // Times are stored as a 32-bit signed int value indicating the number
        // of ms from 12:00AM.  The value is nonnegative.
        return LocalTime.ofNanoOfDay(readInt(position) * 1000000L);
    }


    public void writeTime(int position, LocalTime value) {
        // Times are stored as a 32-bit signed int value indicating the number
        // of ms from 12:00AM.  The value is nonnegative.

        // TODO:  May want to identify any truncation issues that can occur
        //        when casting the time to an int.
        writeInt(position, (int) (value.toNanoOfDay() / 1000000L));
    }


    public LocalDateTime readDateTime(int position) {
        // Date/time values are stored and read as a date value and then a
        // time value.
        return LocalDateTime.of(readDate(position), readTime(position + 4));
    }


    public void writeDateTime(int position, LocalDateTime value) {
        // Date/time values are stored and read as a date value and then a
        // time value.
        writeDate(position, value.toLocalDate());
        writeTime(position + 4, value.toLocalTime());
    }
